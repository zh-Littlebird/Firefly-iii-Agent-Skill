...
以上配置默认均为 `true`；设为 `false` 时，`FireflyClient` 会在代码层强制拦截交易提交/更新中会触发隐式新建的引用，不能仅靠提示词绕过。CLI 与 Python API 都会读取并执行这些开关，不允许因为入口不同而绕过。
...
- 任一自动新建开关为 `false` 时，禁止臆造账户/分类/预算/标签 ID；若要用 ID，必须来自刚刚读取到的列表/自动补全结果
...
- 更新拆分交易时，若一次 `update` 提交多个 split，必须为每个 split 一并提交 `transaction_journal_id`；缺任何一个都不要提交
...
|------|---------|
| 图片模糊无法识别 | 请求用户提供清晰图片 |
| 账户/分类/标签/预算/存钱罐不存在 | 根据自动新建开关处理；开关为 `false` 时必须先展示现有候选项并让用户改选，不能继续提交 |
| 批量更新交易 | `bulk-update` 目前只支持 `where.account_id` / `update.account_id`；不要拿它改分类、预算、标签 |
